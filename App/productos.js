    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="input-group justify-content-center">
                <span class="input-group-text">$ ${numeral(precio).format(
                  "0,0"
                )}</span>
...
const productos_por_keywords = async (keywords, offset) => {
  ...
  try {
    const response = await fetch(
      `https://api.mercadolibre.com/sites/MLA/search?category=${categoria_seleccionada}&q=${keywords}&limit=${productos_por_pagina}&offset=${offset}`
    );
    const result = await response.json();
    // Renderizado
    el_productos.innerHTML = "";
...
function paginado(cantidad_items, pagina_actual, keywords = false) {
  ...
  if (pagina_actual > 0) {
    html += `
                <li class="page-item">
                    <a class="page-link btn_paginado" href="#" data-offset="${
                      pagina_actual - 1
                    }" data-keywords="${keywords}">Anterior</a>
                </li>`;
  }
  html += `
  ...
  if (pagina_actual + 1 < cantidad_de_paginas) {
    html += `
                <li class="page-item">
                    <a class="page-link btn_paginado" href="#" data-offset="${
                      pagina_actual + 1
                    }" data-keywords="${keywords}">Siguiente</a>
                </li>`;
  }
  html += `
        </ul>`;
  ...
  botones_paginado.forEach((boton) => {
    boton.addEventListener("click", (e) => {
      ...
      keywords
        ? productos_por_keywords(keywords, offset)
        : productos_por_categoria(categoria_seleccionada, offset);
    });
  });
}
